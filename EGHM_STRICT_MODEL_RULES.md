...
14. **Accuracy gate**: training nRMSE <=2.0% and fixed-candidate inner LOOCV nRMSE <=2.0% are both required after all physical gates pass.
15. **Ecological trend guard**: the ecological state must have |corr(state, year)| <0.99 for the calibration years used in that fold; otherwise it is treated as an implicit time-trend surrogate.
...
20. **Report order** after substantive runs: RMSE and nRMSE first, then fixed-candidate CV, nested selection CV, water-balance errors, rule-gate result, year-wise predictions, and diagnostics.
...
25. **No arbitrary within-day loss priority**: surface evaporation, surface drainage and subsurface leakage that act during the same daily time step must be evaluated from a common pre-loss surface state. If their combined potential loss exceeds available storage, scale the concurrent losses proportionally rather than allowing implementation order to decide which process gets the water first.
26. **Nested model-selection validation is mandatory**: an evaluation year removed for outer LOOCV may not influence structural parameter choice, ecological rate, antecedent-window choice, Kc, Kh, feature choice, or stopping criteria. Candidate selection must occur entirely inside the remaining years.
27. **Conditional LOOCV is not independent validation**: refitting only Kc/Kh while retaining hyperparameters selected with all six years may be reported as fixed-candidate/inner CV only, never as validation of the complete model-selection procedure.
28. **Nested selection gate**: the six predictions generated by the outer hidden-year procedure must themselves have nRMSE <=2.0%. If any outer fold has no rule-valid inner candidate, the model-selection procedure fails.
29. **Calibration-dimensionality warning**: with only six pond-area target years, multi-parameter hydrologic/geometry searches remain calibration-limited even when nested CV passes. Parameters should be moved from 'calibrated' to 'measured/fixed' as drone DSM, field depth, or independent hydrologic observations become available.
