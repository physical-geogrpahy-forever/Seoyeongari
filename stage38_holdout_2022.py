#!/usr/bin/env python3
"""2022 holdout for the already-locked Stage38 candidate."""
import json
...
from stage38_domain_corrected import hydro,state

SRC=Path('stage38_outputs/stage38_summary.json');OUT=Path('stage38_holdout_outputs');OUT.mkdir(exist_ok=True)
OBS2022=1988.560

def main():
    d=json.loads(SRC.read_text(encoding='utf-8'));c=d.get('selected')
    if not c:raise SystemExit('No locked strict-pass Stage38 candidate')
    ...
    st=state(h['area'],c['r_est_yr']);m22=(yr==2022)&np.isin(mo,[5,6]);S22=float(np.mean(st[m22]))
    ...
    out={'model':'Stage38 locked 2022 holdout','fit_used_2022':False,'selection_changed_after_seeing_2022':False,
         ...
         'locked_candidate':c}
    (OUT/'stage38_holdout_2022.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(out,ensure_ascii=False,indent=2))
