#!/usr/bin/env python3
"""Locked 2022 holdout for a fully strict-passing Stage47 model."""
import json
...
from stage47_habitat_occupancy_recruitment import EST_WINDOW_D,FLOOD_LAG_D,occupancy_state

SRC=Path('stage47_outputs/stage47_summary.json')
OUT=Path('stage47_holdout_outputs'); OUT.mkdir(exist_ok=True)
OBS2022=1988.560

def main():
    d=json.loads(SRC.read_text(encoding='utf-8'))
    if not d.get('nested_selection_pass',False):
        raise SystemExit('Stage47 did not fully pass; 2022 remains sealed')
    ...
    x,te,tr,mr=occupancy_state(E,Fw,c['r_est_yr'],c['r_flood_yr']); S22=float(np.mean(x[m22]))
    ...
    out={'model':'Stage47 locked 2022 holdout','fit_used_2022':False,'selection_changed_after_seeing_2022':False,
    ...
    (OUT/'stage47_holdout_2022.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
