#!/usr/bin/env python3
"""Locked 2022 holdout for a fully strict-passing Stage44 model."""
import json
...
from stage44_continuous_ponding_reversal import continuous_ponding_state

SRC=Path('stage44_outputs/stage44_summary.json');OUT=Path('stage44_holdout_outputs');OUT.mkdir(exist_ok=True);OBS2022=1988.560

def main():
    d=json.loads(SRC.read_text(encoding='utf-8'))
    if not d.get('nested_selection_pass',False):raise SystemExit('Stage44 did not fully pass; 2022 remains sealed')
    ...
    z=continuous_ponding_state(h['area'],c['r_est_yr'],c['r_flood_yr']);S22=float(np.mean(z['state'][m]))
    ...
    out={'model':'Stage44 locked 2022 holdout','fit_used_2022':False,'selection_changed_after_seeing_2022':False,
    ...
    (OUT/'stage44_holdout_2022.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(out,ensure_ascii=False,indent=2))
