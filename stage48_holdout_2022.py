#!/usr/bin/env python3
"""Locked 2022 holdout for a fully strict-passing Stage48 model."""
import json
...
from stage48_nested_recruitment_window import FLOOD_LAG_D

SRC=Path('stage48_outputs/stage48_summary.json')
OUT=Path('stage48_holdout_outputs'); OUT.mkdir(exist_ok=True)
OBS2022=1988.560

def main():
    d=json.loads(SRC.read_text(encoding='utf-8'))
    if not d.get('nested_selection_pass',False):
        raise SystemExit('Stage48 did not fully pass; 2022 remains sealed')
    ...
    a=np.asarray(h['area'],float); exposed=np.clip((A0-a)/A0,0,1); ew=int(c['est_window_d'])
    ...
    hw=int(c['hydro_window_d']); rr=pd.Series(h['return_flow'],index=dt).rolling(hw,min_periods=1).sum().to_numpy()
    ...
    out={'model':'Stage48 locked 2022 holdout','fit_used_2022':False,'selection_changed_after_seeing_2022':False,
      ...
      'establishment_continuous_exposure_window_days':ew,'flood_reversal_trailing_window_days':FLOOD_LAG_D,
    ...
    (OUT/'stage48_holdout_2022.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
